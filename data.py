...
class MiniImagenetLoader(data.Dataset):
    ...
    def get_task_batch(self,
        ...
        query_data = torch.stack([torch.from_numpy(data).float().cuda() for data in query_data], 1)
        query_label = torch.stack([torch.from_numpy(label).float().cuda() for label in query_label], 1)
